...
def database_url() -> str:
    """Build the URL from discrete settings so the password can be a secret injection."""
...
if url.startswith("sqlite"):
    # Lets the HTTP test suite share one in-memory database across TestClient threads.
...
async def lifespan(_: FastAPI):
    # create_all plus a deterministic seed keeps every disposable database usable.
...
app = FastAPI(title=os.getenv("SERVICE_NAME", "catalog-service"), lifespan=lifespan)
